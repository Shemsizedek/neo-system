...
define('NEO_GOOGLE_AI_VERSION', '1.0.0');
...
function neo_google_ai_settings_page() {
    ...
    <div class="wrap">
      ...
      <p><strong>Gemini credential:</strong> define <code>NEO_GEMINI_API_KEY</code> in <code>wp-config.php</code> or provide <code>GEMINI_API_KEY</code> as a server environment variable.</p>
      <form method="post" action="options.php">
        ...
        <table class="form-table"><tbody>
          ...
          <tr><th>Google Labs apps JSON</th><td><textarea class="large-text code" rows="10" name="neo_google_ai_settings[apps_json]"><?php echo esc_textarea(wp_json_encode($o['apps'], JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES)); ?></textarea><p class="description">Array items: slug, name, url, type, description.</p></td></tr>
          <tr><th>Gems JSON</th><td><textarea class="large-text code" rows="10" name="neo_google_ai_settings[gems_json]"><?php echo esc_textarea(wp_json_encode($o['gems'], JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES)); ?></textarea><p class="description">Use url for the official Gem/Labs launch page. Optional instructions create a local Gemini-powered mirror mode.</p></td></tr>
        </tbody></table>
        <?php submit_button(); ?>
      </form>
...
function neo_google_ai_register_rest() {
    register_rest_route(NEO_GOOGLE_AI_REST, '/status', array(
        'methods' => 'GET', 'permission_callback' => '__return_true', 'callback' => 'neo_google_ai_status'
    ));
    register_rest_route(NEO_GOOGLE_AI_REST, '/catalog', array(
        'methods' => 'GET', 'permission_callback' => '__return_true', 'callback' => 'neo_google_ai_catalog'
    ));
    register_rest_route(NEO_GOOGLE_AI_REST, '/generate', array(
        'methods' => 'POST', 'permission_callback' => 'neo_google_ai_rest_permission', 'callback' => 'neo_google_ai_generate'
    ));
}
...
function neo_google_ai_status() {
    ...
    return rest_ensure_response(array(
        'ok' => true,
        'version' => NEO_GOOGLE_AI_VERSION,
        'geminiConfigured' => (bool) neo_google_ai_api_key(),
        'opalBridge' => $o['opal_bridge_url'],
        'defaultModel' => $o['default_model'],
        'labsApps' => count($o['apps']),
        'gems' => count($o['gems']),
    ));
}
...
function neo_google_ai_generate(WP_REST_Request $request) {
    ...
    $system = '';
    if ($gem_slug) {
        foreach ($o['gems'] as $gem) {
            if (($gem['slug'] ?? '') === $gem_slug) { $system = (string)($gem['instructions'] ?? ''); break; }
        }
    }
...
function neo_google_ai_enqueue() {
    ...
    wp_localize_script('neo-google-ai', 'NEOGoogleAI', array(
        'generateUrl' => esc_url_raw(rest_url(NEO_GOOGLE_AI_REST . '/generate')),
...
function neo_gemini_chat_shortcode($atts = array()) {
    ...
    <section class="neo-google-ai-chat" data-model="<?php echo esc_attr($atts['model']); ?>" data-gem="<?php echo esc_attr(sanitize_key($atts['gem'])); ?>">
      ...
      <div class="neo-google-ai-log" aria-live="polite"></div>
...
function neo_google_ai_hub_shortcode() {
    ...
    <section class="neo-google-ai-hub"><header><h2>NEO Google AI Hub</h2><p>Gemini • Gems • Google Labs • Opal</p></header>
      <div class="neo-google-ai-grid">
...
function neo_google_gem_shortcode($atts = array()) {
    ...
    $slug = sanitize_key($atts['slug']); $item = neo_google_ai_find_item('gems', $slug); if (!$item) return '';
