...
export function createTempleCitizenGISSService({ registry, now = () => new Date().toISOString() } = {}) {
  ...
  async function resolve(subject) {
    ...
    if (!credential || credential.status !== 'active') throw new TempleCitizenGISSError('active_neopass_required', 'An active NEOpass credential is required.', 403);
    const citizen = await registry.getTempleCitizen(credential.templeCitizenId);
    if (!citizen || citizen.status !== TEMPLE_CITIZEN_STATUS.ACTIVE) throw new TempleCitizenGISSError('temple_citizen_required', 'Active Temple Citizen status is required.', 403);
    const bookOfLife = await registry.getBookOfLifeRecord(citizen.id);
    if (!bookOfLife || bookOfLife.status !== BOOK_OF_LIFE_STATUS.ACTIVE) throw new TempleCitizenGISSError('book_of_life_record_required', 'An active Book of Life record is required for GISS eligibility.', 403);
    return { credential, citizen, bookOfLife };
  }

  function safeIdentity(identity) {
    return {
      templeCitizen: { id: identity.citizen.id, status: identity.citizen.status },
      bookOfLife: { status: identity.bookOfLife.status },
      neopass: { status: identity.credential.status }
    };
  }

  async function readState(subject) {
    const identity = await resolve(subject);
    const enrollment = await registry.getGISSEnrollment(identity.citizen.id);
    if (!enrollment) {
      return { ...safeIdentity(identity), enrollment: null, degreeAssignment: null, lms: { dashboardState: 'eligible_not_enrolled', lessons: [], assignments: [], nousPortfolio: null, councilAdvancement: null }, timestamp: now() };
    }
    const degreeAssignment = await registry.getTempleDegreeAssignment(enrollment.id);
    return {
      ...safeIdentity(identity), enrollment, degreeAssignment: degreeAssignment || null,
      lms: {
  ...
  return {
    async eligibility(subject) {
      const identity = await resolve(subject);
      const enrollment = await registry.getGISSEnrollment(identity.citizen.id);
      return { eligible: true, entitlement: 'giss.enroll', templeCitizenId: identity.citizen.id, enrollment: enrollment ? { id: enrollment.id, status: enrollment.status } : { status: GISS_ENROLLMENT_STATUS.ELIGIBLE }, timestamp: now() };
    },
    readState,
    async provisionEnrollment(subject) {
      const identity = await resolve(subject);
      let enrollment = await registry.getGISSEnrollment(identity.citizen.id);
      if (!enrollment) enrollment = await registry.createGISSEnrollment({ templeCitizenId: identity.citizen.id, status: GISS_ENROLLMENT_STATUS.ENROLLED, entitlement: 'giss.enroll', createdAt: now() });
      return readState(subject);
    }
  };
}

export function createInMemoryTempleRegistry(seed = {}) {
  const state = { credentials: new Map(seed.credentials || []), citizens: new Map(seed.citizens || []), bookOfLife: new Map(seed.bookOfLife || []), enrollments: new Map(seed.enrollments || []), degreeAssignments: new Map(seed.degreeAssignments || []), portfolios: new Map(seed.portfolios || []), advancements: new Map(seed.advancements || []) };
  return {
    ...
    async createGISSEnrollment(input) { const enrollment = { id: `giss:${input.templeCitizenId}`, ...input }; state.enrollments.set(input.templeCitizenId, enrollment); return enrollment; },
