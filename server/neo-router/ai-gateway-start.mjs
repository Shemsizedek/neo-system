...
const museProjectStore = createMuseProjectStore({
  projectId: process.env.GCP_PROJECT_ID || process.env.GOOGLE_CLOUD_PROJECT,
  databaseId: process.env.FIRESTORE_DATABASE_ID || '(default)',
})
...
createNeoAiGatewayServer({ resolveTrustedIdentity, conversationStore, providerTelemetryStore, museProjectStore, router }).listen(port, host, () => {
