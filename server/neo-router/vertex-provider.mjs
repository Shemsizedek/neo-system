...
async function accessToken(fetchImpl, explicitToken) {
  ...
  if (!response.ok || !body.access_token) throw new Error(`Unable to obtain Google Cloud service-account token (${response.status})`)
  return body.access_token
}

export function createVertexGeminiAdapter({ projectId, location = 'us-central1', model = 'gemini-2.5-flash', accessToken: explicitToken, fetchImpl = fetch, timeoutMs = 30_000 } = {}) {
  return {
    id: 'gemini',
    ...
    async invoke({ system, prompt, maxTokens = 2048 }) {
      ...
      return { provider: 'gemini', transport: 'vertex-ai', model, text, raw: body }
    },
  }
}
