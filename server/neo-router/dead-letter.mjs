...
export function createMemoryDeadLetterQueue(){const items=[];return Object.freeze({mode:'memory',durable:false,async push(item){items.unshift(structuredClone(item));return item},async list(limit=50){return items.slice(0,limit).map(structuredClone)},async clear(){items.splice(0);return true}})}
