...
async function request(url,options={}){const r=await fetch(url,{...options,headers:{'user-agent':'neo-teller/0.5',...(options.headers||{})}});if(!r.ok)throw new Error(`${r.status} ${r.statusText}: ${(await r.text()).slice(0,300)}`);return r}
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='GET'&&req.url==='/health')return json(res,200,{ok:true,service:'neo-teller-backend',mode:'NON_CUSTODIAL_EXPLICIT_BROADCAST'})
  if(req.method==='GET'&&req.url==='/api/v1/teller/network'){
    ...
    return json(res,200,{observedAt:new Date().toISOString(),counterparty,bitcoin,assets:{NOMNI,XCP},capabilities:{readOnly:true,compose:true,sign:false,broadcast:'EXPLICIT_ONLY',walletHandoff:true,confirmationTracking:true}})
  }
...
server.listen(PORT,()=>console.log(`NEO Teller explicit-broadcast gateway listening on :${PORT}`))
