...
import {issueSession} from './auth.mjs';
...
test('Google NEOpass creates a secure session and unlocks the CES dashboard',async()=>running({store,sessionSecret:'session-secret',sessionAudience:'neo-bank',googleClientId:'client-1',executiveAdminEmail:'neo@example.test',verifyGoogleCredential:async()=>({sub:'user-1',email:'neo@example.test',email_verified:true,name:'NEO'})},async base=>{
...
test('CES application routes reject unauthenticated access',async()=>running({store,sessionSecret:'session-secret',sessionAudience:'neo-bank'},async base=>{
...
test('executive admin can govern CES credit limits',async()=>running({store,sessionSecret:'session-secret',sessionAudience:'neo-bank',googleClientId:'client-1',executiveAdminEmail:'neo@example.test',verifyGoogleCredential:async()=>({sub:'user-1',email:'neo@example.test',email_verified:true,name:'NEO'})},async base=>{
...
test('tokens with wrong audience are rejected',async()=>running({store,sessionSecret:'session-secret',sessionAudience:'neo-bank'},async base=>{
  const wrongAudienceToken=issueSession({subject:'google:user-1',email:'attacker@example.test',name:'Attacker',role:'executive-admin'},{secret:'session-secret',issuer:'neo-pass',audience:'giss'});
  const cookie=`neo_bank_session=${encodeURIComponent(wrongAudienceToken)}`;
  assert.equal((await fetch(`${base}/api/v1/me`,{headers:{cookie}})).status,401);
  assert.equal((await fetch(`${base}/api/v1/admin/accounts/CES-1234567890`,{method:'PATCH',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({creditLimit:999})})).status,401);
}));
