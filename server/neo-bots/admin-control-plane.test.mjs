...
test('deployment credential provider reads exchange-scoped environment secrets', async()=>{
  const env={NEO_CES_NMNI_USERNAME:'NMNI0000',NEO_CES_NMNI_PASSWORD:'vault-only',NEO_CES_NMNI_LOGIN_PATH:'/legacy-login.asp'};
  ...
  assert.deepEqual(cesSecretRequirements('NMNI').required,['NEO_CES_NMNI_USERNAME','NEO_CES_NMNI_PASSWORD']);
});
