...
test('only online verified miner shares with accepted pool receipt are accounting eligible',()=>{
  ...
  const result=verifyStratumShare({miner,share:{shareId:'S1',minerId:'M1',contractId:'C1',allocationId:'A1',difficulty:10,simulation:false},poolReceipt:{shareId:'S1',poolId:'P1',accepted:true,receivedAt:new Date().toISOString()}})
...
test('simulation shares never reach customer accounting',()=>{
  ...
  assert.throws(()=>verifyStratumShare({miner,share:{shareId:'S1',minerId:'M1',contractId:'C1',allocationId:'A1',simulation:true},poolReceipt:{shareId:'S1',poolId:'P1',accepted:true,receivedAt:new Date().toISOString()}}),/SIMULATION_SHARE_BLOCKED/)
})
