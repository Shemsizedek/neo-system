...
import {acknowledgeIncident,resolveIncident,incidentBlocksPayout,incidentContext,incidentSummary} from './incidents.mjs'
...
const recoveryIncidents=hydrateMap(state,'recovery_incident')
...
const blockingIncidentsFor=payoutId=>[...recoveryIncidents.values()].filter(i=>i.payoutId===payoutId&&i.state!=='RESOLVED')
const assertIncidentClear=(payoutId,action)=>{if(incidentBlocksPayout([...recoveryIncidents.values()],payoutId))throw new Error(`PAYOUT_HELD_BY_RECOVERY_INCIDENT:${action}`)}
...
export const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='GET'&&req.url==='/health'){const configured=evaluateProductionReadiness(configFromEnv());return json(res,200,{service:'neo-miner-production',status:'UP',mode:configured.mode,storage:{engine:'SQLITE',persistent:true},incidents:incidentSummary([...recoveryIncidents.values()]),time:new Date().toISOString()})}
  ...
  if(req.method==='GET'&&req.url==='/incidents'){if(!authorized(req))return json(res,401,{error:'unauthorized'});const incidents=[...recoveryIncidents.values()];return json(res,200,{summary:incidentSummary(incidents),incidents:incidents.map(i=>incidentContext({incident:i,payout:payoutRequests.get(i.payoutId)||null,psbts:[...psbts.values()],finalizedTransactions:[...finalizedTransactions.values()]}))})}
  const incidentMatch=req.url?.match(/^\/incidents\/([^/]+)\/(acknowledge|resolve)$/)
  if(req.method==='POST'&&incidentMatch){if(!authorized(req))return json(res,401,{error:'unauthorized'});try{const [,id,action]=incidentMatch;const current=recoveryIncidents.get(id);if(!current)throw new Error('INCIDENT_NOT_FOUND');const body=await readBody(req);const updated=action==='acknowledge'?acknowledgeIncident(current,body):resolveIncident(current,body);setPersistent(recoveryIncidents,'recovery_incident',updated.id,updated,action==='acknowledge'?'RECOVERY_INCIDENT_ACKNOWLEDGED':'RECOVERY_INCIDENT_RESOLVED');state.appendAudit('payout',updated.payoutId,action==='acknowledge'?'PAYOUT_HOLD_ACKNOWLEDGED':'PAYOUT_HOLD_RELEASED',{incidentId:updated.id,operatorId:body.operatorId,resolutionCode:body.resolutionCode||null});return json(res,200,incidentContext({incident:updated,payout:payoutRequests.get(updated.payoutId)||null,psbts:[...psbts.values()],finalizedTransactions:[...finalizedTransactions.values()]}))}catch(error){return json(res,409,{error:error.message})}}
  ...
  if(req.method==='POST'&&payoutMatch){if(!authorized(req))return json(res,401,{error:'unauthorized'});try{const [,id,action]=payoutMatch;let p=payoutRequests.get(id);if(!p)throw new Error('PAYOUT_NOT_FOUND');const body=await readBody(req);if(['prepare','sign','broadcast','psbt','finalize-psbt','broadcast-core'].includes(action))assertIncidentClear(p.id,action)
