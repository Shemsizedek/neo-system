...
const json=(res,status,body,extra={})=>{res.writeHead(status,{'content-type':'application/json','cache-control':'no-store','access-control-allow-origin':ORIGIN,'access-control-allow-credentials':'true','vary':'Origin',...extra});res.end(JSON.stringify(body))}
...
const auth=createOperatorAuth({secret:process.env.NEO_OPERATOR_SESSION_SECRET,accounts:parseOperatorAccounts(),ttlSeconds:Number(process.env.NEO_OPERATOR_SESSION_TTL_SEC||1800),secure:process.env.NEO_OPERATOR_COOKIE_SECURE!=='false'})
...
export const operatorServer=http.createServer(async(req,res)=>{
  ...
  if(req.method==='GET'&&req.url==='/health')return json(res,200,{service:'neo-miner-operator-control-plane',status:'UP',auth:'SESSION_RBAC'})
...
if(import.meta.url===`file://${process.argv[1]}`)operatorServer.listen(PORT,'0.0.0.0',()=>console.log(`NEO Miner operator control plane listening on ${PORT}`))
