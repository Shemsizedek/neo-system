...
test('open and acknowledged incidents block payout until resolved',()=>{
  ...
  const resolved=resolveIncident(ack,{operatorId:'op-2',resolutionCode:'CHAIN_VERIFIED',note:'tx absent and safe to resume signing'})
  assert.equal(resolved.state,'RESOLVED')
  assert.equal(incidentBlocksPayout([resolved],'PAY-1'),false)
})

test('incident actions require operator attribution and resolution code',()=>{
  assert.throws(()=>acknowledgeIncident(base,{}),/OPERATOR_ID_REQUIRED/)
  assert.throws(()=>resolveIncident(base,{operatorId:'op-1'}),/RESOLUTION_CODE_REQUIRED/)
})
