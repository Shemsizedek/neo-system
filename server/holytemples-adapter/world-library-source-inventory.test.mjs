...
test("authorized source root is explicitly read-only", () => {
  assert.equal(WORLD_LIBRARY_SOURCE_ROOT.mode, "read-only");
  assert.equal(AUTHORIZED_COLLECTIONS.length, 7);
  assert.ok(BOOK_COLLECTIONS.length >= 14);
});

test("verified recursive leaf scan becomes REVIEW catalog candidates", () => {
  assert.equal(DISCOVERED_SOURCE_FILES.length, 11);
  const hits = libraryCatalogSearch("Millennium Book");
  assert.equal(hits.length, 2);
  assert.ok(hits.every((item) => item.status === "REVIEW"));
