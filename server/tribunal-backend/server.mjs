...
const hits=new Map()
function json(res,status,body,headers={}){res.writeHead(status,{'content-type':'application/json','access-control-allow-origin':'*','access-control-allow-headers':'authorization,content-type,x-neo-signature','access-control-allow-methods':'GET,POST,PUT,PATCH,OPTIONS',...headers});res.end(typeof body==='string'?body:JSON.stringify(body))}
...
const server=createServer(async(req,res)=>{
  ...
  try{
    if(req.method==='GET'&&url.pathname==='/health')return json(res,200,{ok:true,service:'neo-tribunal-backend',version:'1.7',schema:4,time:new Date().toISOString(),communicationsWorker:'scheduler-ready',serviceAutomation:'ready',serviceLedger:'hash-chained',serviceCompliance:'console-ready'})
    ...
    p=match(url.pathname,'/v1/workspaces/:workspaceId/communications/adapters');if(p&&req.method==='GET')return json(res,200,{items:adapterStatus(db,service,principal,p.workspaceId)})
    ...
    p=match(url.pathname,'/v1/workspaces/:workspaceId/communications/outbox/:id/receipt');if(p&&req.method==='POST')return json(res,200,acknowledgeProviderReceipt(db,service,principal,p.workspaceId,p.id,await body(req)))
    ...
    p=match(url.pathname,'/v1/workspaces/:workspaceId/communications/worker/run');if(p&&req.method==='POST')return json(res,200,runCommunicationWorker(db,service,principal,p.workspaceId,await body(req)))
    ...
    p=match(url.pathname,'/v1/workspaces/:workspaceId/communications/provider-health');if(p&&req.method==='GET')return json(res,200,providerHealth(db,service,principal,p.workspaceId))
    ...
    p=match(url.pathname,'/v1/workspaces/:workspaceId/service/compliance/:claimNo/timeline');if(p&&req.method==='GET')return json(res,200,{items:serviceTimeline(db,service,principal,p.workspaceId,p.claimNo)})
    p=match(url.pathname,'/v1/workspaces/:workspaceId/service/compliance/:claimNo/escalations');if(p&&req.method==='POST')return json(res,200,evaluateServiceEscalations(db,service,principal,p.workspaceId,p.claimNo,await body(req)))
    p=match(url.pathname,'/v1/workspaces/:workspaceId/service/compliance/:claimNo/proof-packet');if(p&&req.method==='GET')return json(res,200,buildProofPacket(db,service,principal,p.workspaceId,p.claimNo))
    p=match(url.pathname,'/v1/workspaces/:workspaceId/service/compliance/:claimNo/recommendations');if(p&&req.method==='POST')return json(res,200,complianceRecommendations(db,service,principal,p.workspaceId,p.claimNo,await body(req)))
    p=match(url.pathname,'/v1/workspaces/:workspaceId/service/sync-receipts');if(p&&req.method==='POST')return json(res,200,syncCommunicationReceipts(db,service,principal,p.workspaceId,await body(req)))
    p=match(url.pathname,'/v1/workspaces/:workspaceId/service/alternate-plan');if(p&&req.method==='POST')return json(res,201,alternateServicePlan(db,service,principal,p.workspaceId,await body(req)))
...
server.listen(port,()=>console.log(JSON.stringify({level:'info',event:'server_started',service:'neo-tribunal-backend',version:'1.7',schema:4,communicationsWorker:'scheduler-ready',serviceAutomation:'ready',serviceLedger:'hash-chained',serviceCompliance:'console-ready',port})))
