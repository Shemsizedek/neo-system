...
export function serviceTimeline(db,service,principal,workspaceId,claimNo){
  ...
  for(const r of recipients){
    ...
    const attempts=db.prepare('SELECT * FROM service_attempts WHERE workspace_id=? AND recipient_id=? ORDER BY created_at').all(workspaceId,r.id)
    for(const a of attempts)events.push({at:a.created_at,type:'SERVICE_ATTEMPT',recipientId:r.id,status:a.status,detail:a.error||a.provider_ref||''})
    const proofs=db.prepare('SELECT * FROM proof_of_service WHERE workspace_id=? AND recipient_id=? ORDER BY created_at').all(workspaceId,r.id)
    for(const p of proofs)events.push({at:p.created_at,type:'PROOF_ISSUED',recipientId:r.id,status:'PROOF',detail:p.proof_type})
  }
...
export function evaluateServiceEscalations(db,service,principal,workspaceId,claimNo,{graceHours=24,maxAttempts=3}={}){
  ...
  for(const r of rows){
    const attempts=db.prepare('SELECT COUNT(*) AS n FROM service_attempts WHERE workspace_id=? AND recipient_id=?').get(workspaceId,r.id).n
  ...
  return{workspaceId,claimNo,alerts,generatedAt:now()}
}

export function buildProofPacket(db,service,principal,workspaceId,claimNo){
  service.authorize(principal,workspaceId,'REVIEWER')
  const recipients=db.prepare('SELECT * FROM service_recipients WHERE workspace_id=? AND claim_no=? ORDER BY created_at').all(workspaceId,claimNo)
  const proofs=db.prepare('SELECT * FROM proof_of_service WHERE workspace_id=? AND claim_no=? ORDER BY created_at').all(workspaceId,claimNo)
