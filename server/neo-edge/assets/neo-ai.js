(() => {
  ...
  class NeoTempleAI extends HTMLElement {
    ...
    render() {
      ...
      this.root.innerHTML = `<style>${styles}</style><section class="shell ${this.hasAttribute('fullscreen')?'fullscreen':''}">
        ...
        <form novalidate>
          ...
          <div class="project-transfer">
            <div class="label">Muse / NEO Project Transfer</div>
            <div class="row" style="margin:8px 0"><select class="select" data-project-select><option value="">No project selected</option></select><button class="button secondary" type="button" data-new-project>+ Project</button></div>
            <div class="actions"><button class="button secondary" type="button" data-record-inbound>Record Muse → NEO</button><button class="button secondary" type="button" data-record-outbound>Record NEO → Muse</button></div>
            <div class="handoff-meta" data-project-change>No transfer history yet.</div>
            <div class="lineage" data-project-lineage></div>
          </div>
          <div class="handoff">
      ...
      this.projectChange = this.root.querySelector('[data-project-change]');
      this.projectLineage = this.root.querySelector('[data-project-lineage]');
      this.projects = [];
      ...
      this.root.querySelector('[data-new-project]').addEventListener('click', () => this.createProject());
      this.root.querySelector('[data-record-inbound]').addEventListener('click', () => this.recordProjectTransfer('muse-to-neo'));
      this.root.querySelector('[data-record-outbound]').addEventListener('click', () => this.recordProjectTransfer('neo-to-muse'));
      this.projectSelect.addEventListener('change', () => this.loadProjectLineage());
    ...
    async refreshProjects() {
      try {
        const body=await this.api('/api/ai/projects');
        this.projects=body.projects||[];
        const current=this.projectSelect.value;
        this.projectSelect.innerHTML='<option value="">No project selected</option>'+this.projects.map(p=>`<option value="${escapeHtml(p.id)}">${escapeHtml(p.name)}</option>`).join('');
        if(this.projects.some(p=>p.id===current)) this.projectSelect.value=current;
      } catch {}
    }

    async createProject() {
      const name=prompt('Project name','Muse / NEO Project'); if(!name)return;
      try {
        const body=await this.api('/api/ai/projects',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})});
        await this.refreshProjects(); this.projectSelect.value=body.project.id; await this.loadProjectLineage();
      } catch(error){ this.error.textContent=error.message; this.error.classList.remove('hidden'); }
    }

    async loadProjectLineage() {
      const projectId=this.projectSelect.value;
      if(!projectId){this.projectChange.textContent='No transfer history yet.';this.projectLineage.innerHTML='';return;}
      try {
        const body=await this.api(`/api/ai/projects/${encodeURIComponent(projectId)}/transfers`);
        this.projectChange.textContent=body.comparison?.summary||'No transfer history yet.';
        this.projectLineage.innerHTML=(body.transfers||[]).map(t=>`<div class="lineage-item"><b>${escapeHtml(t.direction)}</b> · ${escapeHtml(t.source||'NEOsync')}<br><span class="meta">${escapeHtml(t.summary||'Transfer recorded')} · ${escapeHtml(t.createdAt||'')}</span></div>`).join('');
      } catch(error){this.projectChange.textContent=error.message;}
    }

    async recordProjectTransfer(direction) {
      const projectId=this.projectSelect.value; if(!projectId){this.projectChange.textContent='Select or create a project first.';return;}
      if(!this.threadId){this.projectChange.textContent='Open a NEOsync thread first.';return;}
      try {
        let content='',summary='';
        if(direction==='neo-to-muse'){ const brief=await this.api(`/api/ai/threads/${encodeURIComponent(this.threadId)}/muse-brief`); content=brief.brief||''; summary='NEOsync brief exported toward Muse'; }
        else { const thread=await this.api(`/api/ai/threads/${encodeURIComponent(this.threadId)}`); content=(thread.thread?.handoffContext||thread.thread?.messages?.[0]?.text||''); summary='Muse content imported into NEOsync'; }
        const body=await this.api(`/api/ai/projects/${encodeURIComponent(projectId)}/transfers`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({direction,threadId:this.threadId,content,summary,source:direction==='neo-to-muse'?'NEOsync':'Meta Muse'})});
        this.projectChange.textContent=body.comparison?.summary||'Transfer recorded.'; await this.loadProjectLineage(); await this.refreshProjects();
      } catch(error){this.projectChange.textContent=error.message;}
    }
