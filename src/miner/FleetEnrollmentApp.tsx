...
export function FleetEnrollmentApp(){
  ...
  const load=async()=>{
    if(!API){setError('Production API URL is not configured.');return}
    if(!TOKEN){setError('Operator token is not configured for this build.');return}
    try{const r=await fetch(`${API}/fleet`,{headers:{authorization:`Bearer ${TOKEN}`},cache:'no-store'});if(!r.ok)throw new Error(`Fleet API ${r.status}`);setData(await r.json());setError('')}catch(e){setError(e instanceof Error?e.message:'Fleet API unavailable')}
  }
  ...
  return <main style={{minHeight:'100vh',background:'#020604',color:'#eaffef',padding:24,fontFamily:'Inter,system-ui,sans-serif'}}>
    <div style={{maxWidth:1200,margin:'0 auto'}}>
      ...
      <div style={{marginTop:22,display:'flex',justifyContent:'space-between',gap:20,alignItems:'end',flexWrap:'wrap'}}><div><div style={{fontSize:12,letterSpacing:2,color:'#79ffa0'}}>NEO MINER FLEET</div><h1 style={{fontSize:'clamp(34px,5vw,62px)',margin:'8px 0'}}>Enrollment & Verified Work</h1><p style={{maxWidth:780,color:'#a8bcae'}}>Only miners with verified cryptographic identity, live telemetry, and pool-accepted non-simulation shares can contribute to cloud-mining accounting.</p></div><button onClick={load} style={{display:'inline-flex',gap:8,alignItems:'center',padding:'12px 16px',borderRadius:12,border:'1px solid #355c3f',background:'#0a1d0f',color:'#eaffef'}}><RefreshCw size={16}/> Refresh</button></div>
