...
export function TreasuryControlApp(){
  ...
  const load=async()=>{if(!API){setError('Production API URL is not configured.');return}try{const r=await fetch(`${API}/treasury`,{headers:TOKEN?{Authorization:`Bearer ${TOKEN}`}:{}});if(!r.ok)throw new Error(`Treasury API ${r.status}`);setData(await r.json());setError('')}catch(e){setError(e instanceof Error?e.message:'Treasury API unavailable')}}
