...
import {Activity,ArrowLeft,Bitcoin,Cpu,Globe2,Layers3,RefreshCw,ShieldCheck,Sparkles,WalletCards,Zap} from 'lucide-react'
...
export function GeneratorApp(){
  ...
  return <div className="ng-shell">
    <header className="ng-header"><div><a href="#/miner"><ArrowLeft size={16}/> NEO MINER</a><p>NEO BITCOIN GENERATOR v0.3</p><h1>Dynamic Hashpower Aggregator & Settlement Engine</h1></div><div className="ng-mode"><i/> DEMO / CONTRACT SIMULATION</div></header>

    <section className="ng-hero"><div><span>GENERATOR STATUS</span><h2>Aggregate physical SHA-256 capacity, rebalance failures, verify delivered work, and settle attributable BTC.</h2><p>Digital NEO Miners remain service allocations backed by mining fleet capacity. Settlement is gated by verified share attribution and cannot be released from simulation data.</p></div><div className="ng-core"><Sparkles size={24}/><b>{snapshot.physicalHashrateTh.toFixed(1)} TH/s</b><span>physical demo fleet</span><small>{snapshot.utilizationPct.toFixed(1)}% allocated</small></div></section>
    ...
    <section className="ng-panel ng-architecture"><div className="ng-title"><div><span>GENERATOR PIPELINE</span><h3>Proof-of-Work Backing & Settlement</h3></div><ShieldCheck size={20}/></div><div className="ng-flow"><b>World Currency Payment</b><i>→</i><b>Exchange Order</b><i>→</i><b>Capacity Reservation</b><i>→</i><b>Dynamic Aggregator</b><i>→</i><b>NEO Miner Fleet</b><i>→</i><b>Stratum V2 Shares</b><i>→</i><b>Verified Attribution</b><i>→</i><b>HashVault Settlement</b></div><p>Every settlement must resolve to source miner IDs, delivered hashrate, verified pool/share records, SLA performance, fee calculations, and an auditable settlement batch. Unverified or simulated production cannot settle.</p></section>
  </div>
}
