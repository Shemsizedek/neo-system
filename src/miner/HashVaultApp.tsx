...
export function HashVaultApp(){
  const[data,setData]=useState<Snapshot|null>(null);const[error,setError]=useState('');const[session,setSession]=useState<OperatorSession|null>(null)
  const load=async()=>{if(!session)return;try{const r=await operatorFetch('/hashvault',{cache:'no-store'});if(!r.ok)throw new Error(`HashVault API ${r.status}`);setData(await r.json());setError('')}catch(e){setError(e instanceof Error?e.message:'HashVault unavailable')}}
  ...
  return <main style={{minHeight:'100vh',background:'#020604',color:'#eaffef',padding:24,fontFamily:'Inter,system-ui,sans-serif'}}><div style={{maxWidth:1180,margin:'0 auto'}}>
    ...
    {!session?<p style={{marginTop:24,color:'#a8bcae'}}>Sign in with an authorized operator account to load HashVault data.</p>:<>
