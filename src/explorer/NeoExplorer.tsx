...
import {assessOrangeChipEvidence,listOrangeChipEvidence} from './orangeChipEvidence'
...
export function NeoExplorer(){
  ...
  const evidence=useMemo(()=>listOrangeChipEvidence(selectedAsset),[selectedAsset])
  const assessment=useMemo(()=>assessOrangeChipEvidence(selectedAsset,verification),[selectedAsset,verification,evidence.length])
  ...
  return <div className="neo-explorer">
    ...
    <section className="explorer-grid">
      ...
      <article className="card explorer-panel trade-ticket"><div className="explorer-panel-title"><div><span>ORANGE CHIP™ STOCK INTELLIGENCE</span><h3>{selectedAsset}</h3></div><Activity size={17}/></div><div className="prime-copy"><b>Issuance source</b><p>{selectedIssuance?.source??selectedIssuance?.issuer??'Not reported'}</p><b>Issued quantity</b><p>{issuanceQuantity(selectedIssuance)}</p><b>Central-wallet balance</b><p>{balanceValue(selectedBalance)}</p><b>Issuer / owner</b><p>{assetInfo?.issuer??assetInfo?.owner??'Not reported'}</p><b>Description</b><p>{selectedIssuance?.description||assetInfo?.description||'No on-chain description reported.'}</p><b>Orange Chip™ rule</b><p>Valid Counterparty issuances sourced by the Central Listing Wallet establish Orange Chip™ Stock listing status. Underlying company, credit, equity, royalty, fund, warrant, surety, or other off-chain rights remain separately evidence-verified.</p></div></article>
      <article className="card explorer-panel prime-panel"><div className="explorer-panel-title"><div><span>ORANGE CHIP™ EVIDENCE</span><h3>{assessment.recommendedLevel}</h3></div><Sparkles size={17}/></div><div className="prime-copy"><b>Evidence score</b><p>{assessment.score} / 100</p><b>Accepted</b><p>{assessment.accepted}</p><b>Unreviewed</b><p>{assessment.unreviewed}</p><b>Rejected / conflicted</b><p>{assessment.rejected} / {assessment.conflicted}</p><b>Off-chain claim type</b><p>{verification.claimType}</p><b>Assessment</b>{assessment.rationale.map(note=><p key={note}>{note}</p>)}<small>Evidence ingestion never upgrades verification automatically; the recommendation remains advisory until reviewed and approved.</small></div></article>
    </section>
    ...
    <section className="card registry-panel"><div className="explorer-panel-title"><div><span>ORANGE CHIP™ STOCK REGISTRY</span><h3>Central Listing Wallet Issuances</h3></div><Bitcoin size={17}/></div><div className="registry-table"><div className="registry-head"><span>STOCK</span><span>ISSUED QUANTITY</span><span>MARKET</span><span>STATUS</span></div>{stockIssuances.map(issuance=>{const record=getOrangeChipVerification(issuance.asset??'');const evidenceAssessment=assessOrangeChipEvidence(issuance.asset??'',record);return <button className="registry-row" key={issuance.asset} onClick={()=>issuance.asset&&setSelectedAsset(issuance.asset)}><b>{issuance.asset}</b><span>{issuanceQuantity(issuance)}</span><span>{issuance.asset}/XCP</span><em>{issuance.asset===selectedAsset?'SELECTED':evidenceAssessment.recommendedLevel}</em></button>})}</div></section>
    <section className="explorer-architecture"><div><Bitcoin size={18}/><b>Bitcoin</b><span>Settlement foundation</span></div><div><BookOpen size={18}/><b>Counterparty</b><span>Issuance provenance</span></div><div><Globe2 size={18}/><b>Evidence</b><span>Ingestion + review queue</span></div><div><WalletCards size={18}/><b>NEO DEX</b><span>Per-stock market interface</span></div><div><Sparkles size={18}/><b>NEO Algo</b><span>Advisory evidence assessment</span></div></section>
  </div>
}
