...
export function NeoExplorer(){
  ...
  return <div className="neo-explorer">
    ...
    <section className="explorer-stats"><div><span>SELECTED MARKET</span><b>{pair}</b><small>{feedState==='loading'?'Connecting…':feedState==='live'?'Counterparty feed connected':'Counterparty feed degraded'}</small></div><div><span>BEST BID</span><b>{formatPrice(xcpQuote?.bid)}</b><small>Attributed: XCP DEX</small></div><div><span>BEST ASK</span><b>{formatPrice(xcpQuote?.ask)}</b><small>Attributed: XCP DEX</small></div><div><span>HOLDERS</span><b>{holderCount==null?'—':formatCount(holderCount)}</b><small>{selectedAsset} holder records</small></div><div><span>EVIDENCE VAULT</span><b>{vaultState.toUpperCase()}</b><small>{vaultEvidence.length} persistent record(s)</small></div></section>
