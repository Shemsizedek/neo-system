...
export function NeoExplorer(){
  ...
  return <div className="neo-explorer">
    ...
    <section className="explorer-grid">
      ...
      <article className="card explorer-panel prime-panel"><div className="explorer-panel-title"><div><span>ORANGE CHIP™ VERIFICATION</span><h3>{verification.level}</h3></div><Sparkles size={17}/></div><div className="prime-copy"><b>Verification score</b><p>{score} / 100</p><b>Off-chain claim type</b><p>{verification.claimType}</p><b>Legal entity</b><p>{verification.legalEntity??'Not recorded'}</p><b>Jurisdiction</b><p>{verification.jurisdiction??'Not recorded'}</p><b>Evidence records</b><p>{verification.evidence.length}</p>{verification.notes.map(note=><p key={note}>{note}</p>)}</div></article>
    </section>
    ...
    <section className="card registry-panel"><div className="explorer-panel-title"><div><span>ORANGE CHIP™ STOCK REGISTRY</span><h3>Central Listing Wallet Issuances</h3></div><Bitcoin size={17}/></div><div className="registry-table"><div className="registry-head"><span>STOCK</span><span>ISSUED QUANTITY</span><span>MARKET</span><span>STATUS</span></div>{stockIssuances.map(issuance=>{const record=getOrangeChipVerification(issuance.asset??'');return <button className="registry-row" key={issuance.asset} onClick={()=>issuance.asset&&setSelectedAsset(issuance.asset)}><b>{issuance.asset}</b><span>{issuanceQuantity(issuance)}</span><span>{issuance.asset}/XCP</span><em>{issuance.asset===selectedAsset?'SELECTED':record.level}</em></button>})}</div></section>
    <section className="explorer-architecture"><div><Bitcoin size={18}/><b>Bitcoin</b><span>Settlement foundation</span></div><div><BookOpen size={18}/><b>Counterparty</b><span>Issuance provenance</span></div><div><Globe2 size={18}/><b>Evidence</b><span>Off-chain verification layer</span></div><div><WalletCards size={18}/><b>NEO DEX</b><span>Per-stock market interface</span></div><div><Sparkles size={18}/><b>NEO Algo</b><span>Reasoning + provenance</span></div></section>
  </div>
}
