...
export function NeoExplorer(){
  ...
  return <div className="neo-explorer">
    ...
    <section className="explorer-grid">
      ...
      <article className="card explorer-panel trade-ticket"><div className="explorer-panel-title"><div><span>ASSET INTELLIGENCE CARD</span><h3>{selectedAsset}</h3></div><Activity size={17}/></div><div className="prime-copy"><b>Foundation balance</b><p>{balanceValue(selectedBalance)}</p><b>Issuer / owner</b><p>{assetInfo?.issuer??assetInfo?.owner??'Not reported'}</p><b>Description</b><p>{assetInfo?.description||'No on-chain description reported.'}</p><b>Off-chain classification</b><p>Unverified unless separately documented. Token existence alone does not establish equity, debt, credit, royalty, fund, warrant, surety, or other legal/economic rights.</p></div></article>
      <article className="card explorer-panel prime-panel"><div className="explorer-panel-title"><div><span>NEO PRIME + NEO ALGO</span><h3>Market Intelligence</h3></div><Sparkles size={17}/></div><div className="prime-copy"><b>Evidence synthesis</b><p>NEO Algo evaluates {pair} independently from the Orange Chip™ discovery status and preserves missing CES/external-market data instead of manufacturing prices.</p></div>{observations.map(o=><div className="source-row" key={o.venue}><b>{o.venue}</b><span>{o.status.toUpperCase()}</span></div>)}</article>
    </section>
