...
const marketObservations:MarketObservation[]=[
  {venue:'XCP_DEX',source:'XCP DEX / Counterparty market adapter',status:'unavailable',note:'Live normalized quote is not connected yet.'},
  {venue:'CES',source:'Community Exchange System',status:'unavailable',note:'Authorized CES market-data adapter is pending.'},
  {venue:'EXTERNAL',source:'External exchange adapters',status:'unavailable',note:'No verified external NOMNI venue is configured.'},
  {venue:'NEO_DEX',source:'NEO DEX interface model',status:'illustrative',note:'Visible order-book rows are UI fixtures and are not treated as live price evidence.'},
]

export function NeoExplorer(){
  const algo=useMemo(()=>analyzeExplorerMarket('NOMNI/XCP',marketObservations),[])
  return <div className="neo-explorer">
    ...
    <section className="explorer-stats"><div><span>FLAGSHIP MARKET</span><b>NOMNI / XCP</b><small>NEO DEX prototype pair</small></div><div><span>SETTLEMENT</span><b>BITCOIN</b><small>Non-custodial target architecture</small></div><div><span>ASSET PROTOCOL</span><b>COUNTERPARTY</b><small>Issuance, orders and transfers</small></div><div><span>COMMUNITY MARKET</span><b>CES</b><small>Adapter layer planned</small></div><div><span>INTELLIGENCE</span><b>NEO ALGO</b><small>{algo.result.cycle.toUpperCase()} cycle • {algo.result.risk.toUpperCase()} risk</small></div></section>
    <section className="explorer-grid">
      <article className="card explorer-panel orderbook"><div className="explorer-panel-title"><div><span>NEO DEX ORDER BOOK</span><h3>NOMNI / XCP</h3></div><em>MODEL</em></div><div className="book-cols"><span>PRICE XCP</span><span>AMOUNT NOMNI</span><span>TOTAL XCP</span></div>{asks.map(r=><div className="book-row ask" key={r[0]}><b>{r[0]}</b><span>{r[1]}</span><span>{r[2]}</span></div>)}<div className="spread">SPREAD <b>0.000004 XCP</b></div>{bids.map(r=><div className="book-row bid" key={r[0]}><b>{r[0]}</b><span>{r[1]}</span><span>{r[2]}</span></div>)}<p className="model-note">Displayed order values are illustrative until the live Counterparty DEX feed is connected. NEO Algo excludes these fixture values from verified market confidence.</p></article>
      <article className="card explorer-panel trade-ticket"><div className="explorer-panel-title"><div><span>EXECUTION</span><h3>Trade NOMNI</h3></div><Activity size={17}/></div><div className="trade-tabs"><b>LIMIT</b><span>MARKET</span></div><label>PRICE (XCP)<input defaultValue="0.000076"/></label><label>AMOUNT (NOMNI)<input placeholder="Enter amount"/></label><div className="trade-total"><span>Estimated total</span><b>— XCP</b></div><div className="trade-buttons"><button>BUY</button><button>SELL</button></div><p>Execution remains disabled until the wallet/signing and Counterparty transaction-broadcast layer is connected. NEO Algo classifies financial execution as consequential and requires human authorization.</p></article>
      <article className="card explorer-panel prime-panel"><div className="explorer-panel-title"><div><span>NEO PRIME + NEO ALGO</span><h3>Market Intelligence</h3></div><Sparkles size={17}/></div><div className="prime-copy"><b>Liquidity synthesis</b><p>NEO Prime presents the market view. NEO Algo grounds, reconciles and resolves the evidence while preserving venue attribution and missing-data status.</p></div><div className="source-row"><b>XCP DEX</b><span>Unavailable</span></div><div className="source-row"><b>CES</b><span>Unavailable</span></div><div className="source-row"><b>External Exchanges</b><span>Unavailable</span></div><div className="source-row"><b>NEO DEX</b><span>Illustrative model</span></div></article>
    </section>
