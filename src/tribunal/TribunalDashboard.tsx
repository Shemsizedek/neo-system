...
export function TribunalDashboard(){
  ...
  const attach=()=>{const selectedSource=corpusSources.find(item=>item.sourceId===sourceId);if(!selectedSource)return;setCaseFile(current=>addCitationToCase(current,makeCitation({authorityId,sourceId,locator:selectedSource.locator??selectedSource.title,proposition})))}
  const addExhibit=()=>{if(!evidenceTitle.trim()||!evidenceDescription.trim())return;const exhibit=createEvidence({title:evidenceTitle,kind:evidenceKind,offeredBy:'TRIBUNAL',description:evidenceDescription,tags:['working-record']},caseFile.evidence.length+1);setCaseFile(current=>addEvidenceToCase(current,exhibit));setEvidenceTitle('');setEvidenceDescription('')}
  const admit=(exhibitId:string)=>setCaseFile(current=>{const item=current.evidence.find(e=>e.exhibitId===exhibitId);return item?replaceEvidence(current,setEvidenceStatus(item,'ADMITTED')):current})
  ...
  return <>
    ...
    <section className="card tablecard"><div className="paneltitle"><div><span>Evidence & Exhibit Registry</span><small>Exhibit identity, status and chain-of-custody foundation</small></div><FileText size={18}/></div><div className="tribunalcitegrid"><label>Title<input value={evidenceTitle} onChange={e=>setEvidenceTitle(e.target.value)} placeholder="Exhibit title"/></label><label>Kind<select value={evidenceKind} onChange={e=>setEvidenceKind(e.target.value as EvidenceKind)}><option>DOCUMENT</option><option>AFFIDAVIT</option><option>CORRESPONDENCE</option><option>FINANCIAL_RECORD</option><option>IMAGE</option><option>AUDIO_VIDEO</option><option>TESTIMONY</option><option>OTHER</option></select></label></div><label>Description<textarea value={evidenceDescription} onChange={e=>setEvidenceDescription(e.target.value)} rows={2}/></label><button className="primary" disabled={!evidenceTitle.trim()||!evidenceDescription.trim()} onClick={addExhibit}>Register exhibit</button><div className="tablewrap"><table><thead><tr><th>Exhibit</th><th>Title</th><th>Kind</th><th>Status</th><th>Integrity</th><th>Custody</th><th>Action</th></tr></thead><tbody>{caseFile.evidence.map(item=><tr key={item.exhibitId}><td className="mono">{item.exhibitId}</td><td>{item.title}</td><td>{item.kind}</td><td>{item.status}</td><td>{item.integrity}</td><td>{item.custody.length} events</td><td>{item.status!=='ADMITTED'?<button onClick={()=>admit(item.exhibitId)}>Admit</button>:'—'}</td></tr>)}</tbody></table></div></section>

    <TribunalPacketPanel caseFile={caseFile} setCaseFile={setCaseFile} opinion={opinion}/>
    <section className="card tablecard"><div className="paneltitle"><div><span>Case Authority Matrix</span><small>Authority-source-proposition chain for the record</small></div><CheckCircle2 size={18}/></div><div className="tablewrap"><table><thead><tr><th>Citation</th><th>Authority</th><th>Source</th><th>Proposition</th><th>Integrity</th></tr></thead><tbody>{matrix.length?matrix.map(({citation,validation})=><tr key={citation.citationId}><td className="mono">{citation.citationId}</td><td>{citation.authorityId}</td><td>{citation.sourceId}</td><td>{citation.proposition}</td><td><span className={'pill '+(validation.valid?'completed':'manual_review')}>{validation.valid?'VALID':'REVIEW'}</span></td></tr>):<tr><td colSpan={5}>No Corpus authorities attached to this docket yet.</td></tr>}</tbody></table></div></section>

    <section className="card panel"><div className="paneltitle"><div><span>NEOsync Opinion Builder</span><small>Draft findings and analysis with proposition-level authority and exhibit links</small></div><Gavel size={18}/></div><label>Analysis<textarea rows={5} value={opinion.sections.find(s=>s.id==='SEC-ANALYSIS')?.body??''} onChange={e=>setSection('SEC-ANALYSIS',e.target.value)} placeholder="Analyze the record, distinguish allegations from findings, and explain the authority used."/></label><label>Conclusion<textarea rows={3} value={opinion.sections.find(s=>s.id==='SEC-CONCLUSION')?.body??''} onChange={e=>setSection('SEC-CONCLUSION',e.target.value)} placeholder="State the internal Tribunal conclusion."/></label><button className="primary" onClick={linkRecordToAnalysis}>Link admitted record to analysis</button><div className="route"><span>VALIDATION</span><b>{opinionValidation.valid?'READY FOR INTERNAL REVIEW':`${opinionValidation.problems.length} issue(s)`}</b></div>{!opinionValidation.valid&&<ul>{opinionValidation.problems.map(problem=><li key={problem}>{problem}</li>)}</ul>}<label>Rendered opinion preview<textarea readOnly rows={16} value={renderOpinionText(opinion,caseFile.citations,caseFile.evidence)}/></label></section>
  </>
}
