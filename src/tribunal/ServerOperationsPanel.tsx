...
export function ServerOperationsPanel({caseFile,setCaseFile}:{caseFile:TribunalCase;setCaseFile:(next:TribunalCase)=>void}){
  ...
  const [workspaces,setWorkspaces]=useState<ServerWorkspace[]>([]),[email,setEmail]=useState(''),[password,setPassword]=useState(''),[displayName,setDisplayName]=useState(''),[workspaceName,setWorkspaceName]=useState('World Interfaith Tribunal')
  ...
  const choose=(id:string)=>{const next=workspaces.find(w=>w.id===id)||null;setWorkspace(next);if(next)localStorage.setItem(WORKSPACE_KEY,JSON.stringify(next));setRevision(undefined)}
  ...
  const signOut=async()=>{try{if(session)await api.logout()}catch{}setSession(null);setWorkspace(null);setWorkspaces([]);setRevision(undefined);localStorage.removeItem(SESSION_KEY);localStorage.removeItem(WORKSPACE_KEY);setStatus('Signed out')}
  ...
  return <section className="card panel">
    <div className="paneltitle"><div><span>NEO Tribunal v1.0 Operations</span><small>Authenticated workspaces, synchronized dockets, audit verification and release readiness</small></div><Cloud size={18}/></div>
    ...
    {!session?<><div className="tribunalcitegrid"><label>Email<input value={email} onChange={e=>setEmail(e.target.value)}/></label><label>Password<input type="password" value={password} onChange={e=>setPassword(e.target.value)}/></label></div><label>Display name<input value={displayName} onChange={e=>setDisplayName(e.target.value)}/></label><button onClick={()=>void register()} disabled={!email||password.length<8||!displayName}><KeyRound size={14}/> Register</button> <button className="primary" onClick={()=>void login()} disabled={!email||!password}>Sign in</button></>:<>
      ...
      {!!workspaces.length&&<label>Workspace<select value={workspace?.id||''} onChange={e=>choose(e.target.value)}><option value="">Select workspace</option>{workspaces.map(w=><option key={w.id} value={w.id}>{w.name} — {w.role}</option>)}</select></label>}
