...
export type SignerCapabilities={rawTransaction:boolean;legacy:boolean;segwit:boolean;taproot:boolean}
export type AdapterSession={address:string;network:string;addressType:BitcoinAddressType}
export type BitcoinSignerProvider={name?:string;connect?:()=>Promise<{address:string;network?:string}>;disconnect?:()=>Promise<void>|void;signRawTransaction?:(hex:string)=>Promise<string>;capabilities?:Partial<SignerCapabilities>}
...
function defaults(p:BitcoinSignerProvider):SignerCapabilities{
 const raw=typeof p.signRawTransaction==='function'
 return{rawTransaction:raw,legacy:raw,segwit:raw,taproot:false,...p.capabilities}
}
export class BitcoinSignerAdapter{
 ...
 async connect():Promise<AdapterSession>{
  if(typeof this.provider.connect!=='function'||typeof this.provider.signRawTransaction!=='function')throw new Error('Wallet does not expose the NEOpay Bitcoin signing interface.')
