...
export class NEOpaySigner{
 ...
 async connect(){const a=this.adapter();if(!a){this.session={available:false,connected:false,name:'Connected wallet'};return this.session}const h=a.health();if(!h.available){this.session={available:false,connected:false,name:a.name,capabilities:h.capabilities};return this.session}const s=await a.connect();this.session={available:true,connected:true,name:a.name,address:s.address,network:s.network,addressType:s.addressType,capabilities:h.capabilities};this.armTimer();return{...this.session}}
