...
export class NEOpaySigner{
  ...
  state():SignerState{const p=this.provider();return{available:!!p&&typeof p.signRawTransaction==='function',connected:false,name:p?.name||'Connected wallet'}}
  async connect():Promise<SignerState>{const p=this.provider();if(!p||typeof p.connect!=='function'||typeof p.signRawTransaction!=='function')return{available:false,connected:false,name:p?.name||'Connected wallet'};const session=await p.connect();if(!session?.address)throw new Error('Wallet did not return an address.');return{available:true,connected:true,name:p.name||'Connected wallet',address:session.address,network:session.network||'mainnet'}}
  async disconnect(){await this.provider()?.disconnect?.()}
  async sign(req:SignRequest){const p=this.provider();if(!p||typeof p.signRawTransaction!=='function')throw new Error('Connect a compatible wallet to approve this transaction.');if(!/^[0-9a-fA-F]{100,400000}$/.test(req.unsignedTxHex)||req.unsignedTxHex.length%2)throw new Error('Unsigned transaction payload is invalid.');const signed=await p.signRawTransaction(req.unsignedTxHex);if(!/^[0-9a-fA-F]{100,400000}$/.test(String(signed||''))||signed.length%2)throw new Error('Wallet returned an invalid transaction.');return signed}
}
