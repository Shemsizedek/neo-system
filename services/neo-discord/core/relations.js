...
async function gh(env,path,fetchImpl=fetch){
  ...
  const r=await fetchImpl(`${API}${REPO}${path}`,{headers,signal:AbortSignal.timeout(12000)})
  const b=await r.json().catch(()=>({}))
  if(!r.ok)throw new Error(`GitHub ${r.status}: ${b?.message||'request failed'}`)
  return b
}

async function relationsApi(env,path,fetchImpl=fetch){
  ...
  const r=await fetchImpl(`${base}${path}`,{headers:{accept:'application/json',authorization:`Bearer ${token}`,'user-agent':'neo-relations-discord'},signal:AbortSignal.timeout(12000)})
  const b=await r.json().catch(()=>({}))
...
async function fetchJson(env,path,fetchImpl){const file=await gh(env,`/contents/${path}?ref=main`,fetchImpl);return JSON.parse(decodeContent(file))}
async function fetchTenant(env,tenant,fetchImpl){const safe=tenant==='neopay'?'neopay':'neo-prime';return fetchJson(env,`apps/neo-relations/tenants/${safe}.json`,fetchImpl)}

async function relationsStatus(env,fetchImpl){
  const [repo,prime,pay,control]=await Promise.all([gh(env,'',fetchImpl),fetchTenant(env,'neo-prime',fetchImpl),fetchTenant(env,'neopay',fetchImpl),fetchJson(env,'apps/neo-relations/contracts/control-plane.json',fetchImpl)])
...
async function architectureSummary(env,fetchImpl){
  const c=await fetchJson(env,'apps/neo-relations/contracts/control-plane.json',fetchImpl)
...
async function tenantSummary(env,tenant,fetchImpl){
  const cfg=await fetchTenant(env,tenant,fetchImpl)
...
async function serviceSummary(env,fetchImpl){
  const s=await fetchJson(env,'apps/neo-relations/site/data/status.json',fetchImpl)
...
async function approvalsSummary(env,tenant,fetchImpl){
  const safe=tenant==='neopay'?'neopay':'neo-prime'
  const data=await relationsApi(env,`/intents?tenantId=${encodeURIComponent(safe)}&status=pending_approval`,fetchImpl)
...
export async function handleRelationsCommand(interaction,env,{fetchImpl=fetch}={}){
  const subcommand=interaction?.data?.options?.[0]||{}
  const sub=String(subcommand?.name||'status')
  if(sub==='status')return relationsStatus(env,fetchImpl)
  if(sub==='architecture')return architectureSummary(env,fetchImpl)
  if(sub==='tenant')return tenantSummary(env,subOption(subcommand,'name')||'neo-prime',fetchImpl)
  if(sub==='services')return serviceSummary(env,fetchImpl)
  if(sub==='approvals')return approvalsSummary(env,subOption(subcommand,'tenant')||'neo-prime',fetchImpl)
