...
export async function inspectAdapter(adapter,env){const checkedAt=new Date().toISOString();const configured=adapter.env?cleanBase(env[adapter.env]):'';if(configured){const url=configured+(adapter.healthPath||'/health');try{const r=await timedFetch(url,{},4500);let detail='';try{detail=(await r.text()).slice(0,300)}catch{}return {...adapter,mode:'live-service',state:r.ok?'LIVE':'DEGRADED',httpStatus:r.status,endpoint:configured,checkedAt,detail}}catch(error){return {...adapter,mode:'live-service',state:'OFFLINE',endpoint:configured,checkedAt,error:String(error?.message||error)}}}if(adapter.kind==='government-store')return {...adapter,mode:'native-government-store',state:'LIVE',endpoint:'GOV_STORE',checkedAt};try{const r=await timedFetch(RAW+adapter.repoPath,{},3500);return {...adapter,mode:'repository-engine',state:r.ok?'REPOSITORY':'UNAVAILABLE',httpStatus:r.status,endpoint:RAW+adapter.repoPath,checkedAt}}catch(error){return {...adapter,mode:'repository-engine',state:'UNAVAILABLE',endpoint:RAW+adapter.repoPath,checkedAt,error:String(error?.message||error)}}}
export async function inspectAllAdapters(env){return Promise.all(ADAPTERS.map(a=>inspectAdapter(a,env)))}
export async function proxyAdapter(adapter,env,subpath,request){const base=adapter.env?cleanBase(env[adapter.env]):'';if(!base)return new Response(JSON.stringify({ok:false,error:'Adapter has no configured live service.',adapter:adapter.id,state:'REPOSITORY'}),{status:503,headers:{'content-type':'application/json'}});const target=new URL(base+(subpath.startsWith('/')?subpath:'/'+subpath));const headers=new Headers();for(const[k,v]of request.headers){if(['content-type','accept','if-none-match','if-modified-since'].includes(k.toLowerCase()))headers.set(k,v)}if(env.MODULE_ADAPTER_TOKEN)headers.set('authorization',`Bearer ${env.MODULE_ADAPTER_TOKEN}`);const init={method:request.method,headers,redirect:'manual'};if(!['GET','HEAD'].includes(request.method))init.body=await request.arrayBuffer();const r=await timedFetch(target.toString(),init,12000);const outHeaders=new Headers();outHeaders.set('content-type',r.headers.get('content-type')||'application/json');outHeaders.set('cache-control','no-store');return new Response(r.body,{status:r.status,headers:outHeaders})}
