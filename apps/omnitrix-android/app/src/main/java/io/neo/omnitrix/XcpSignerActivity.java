...
/** Human approval boundary for local Bitcoin/Counterparty transaction signing and separate broadcast. */
public class XcpSignerActivity extends Activity {
    ...
    private static final int REQ_BROADCAST=1803;
    private XcpKeyVault vault;
    private EditText request;
    private TextView preview,result;
    private Button sign,copy,broadcast;
    ...
    @Override public void onCreate(Bundle state){
        ...
        buildUi();
        String incoming=getIntent().getStringExtra("signing_request");
        if(incoming!=null&&!incoming.trim().isEmpty()) request.setText(incoming.trim());
    }
    ...
    private void buildUi(){
        ...
        TextView sub=text("NEO-0001 · Local Signing + Broadcast Boundary v1.9",12,Color.rgb(103,170,126));sub.setPadding(0,dp(5),0,dp(16));root.addView(sub);
        TextView guard=text("Review first. Sign second. Broadcast third. The XCP Key never leaves this device. Broadcasting sends only already-signed Bitcoin transaction hex.",13,Color.rgb(174,211,186));guard.setPadding(0,0,0,dp(14));root.addView(guard);
        ...
        copy=secondary("COPY SIGNED TRANSACTION HEX");copy.setVisibility(View.GONE);root.addView(copy,new LinearLayout.LayoutParams(-1,dp(48)));copy.setOnClickListener(v->{ClipboardManager cm=(ClipboardManager)getSystemService(CLIPBOARD_SERVICE);if(signedHex!=null&&!signedHex.isEmpty()){cm.setPrimaryClip(ClipData.newPlainText("Omnitrix signed transaction",signedHex));Toast.makeText(this,"Signed transaction copied",Toast.LENGTH_SHORT).show();}});
        broadcast=primary("BROADCAST SIGNED TRANSACTION");broadcast.setVisibility(View.GONE);LinearLayout.LayoutParams blp=new LinearLayout.LayoutParams(-1,dp(54));blp.setMargins(0,dp(10),0,0);root.addView(broadcast,blp);broadcast.setOnClickListener(v->confirmBroadcast());
    ...
    private void confirmBroadcast(){
        if(signedHex==null||signedHex.isEmpty()){Toast.makeText(this,"No signed transaction available",Toast.LENGTH_SHORT).show();return;}
        new AlertDialog.Builder(this).setTitle("Broadcast to Bitcoin mainnet?")
                .setMessage("This is the final network action. Once accepted by the Bitcoin network, Omnitrix cannot reverse the transaction.\n\nExpected TXID:\n"+signedTxid)
                .setPositiveButton("Authenticate & Broadcast",(d,w)->authenticate(REQ_BROADCAST)).setNegativeButton("Cancel",null).show();
    }

    private void authenticate(int code){
        if(code!=REQ_BROADCAST&&!vault.hasKey()){Toast.makeText(this,"No secured XCP Key",Toast.LENGTH_SHORT).show();return;}
        ...
        String why=code==REQ_SIGN?"Approve local signing for NEO-0001":code==REQ_BROADCAST?"Final approval to broadcast the signed Bitcoin transaction":"Unlock XCP Key to review ownership and fees";
        Intent i=km.createConfirmDeviceCredentialIntent("Omnitrix XCP",why);
    ...
    private void onAuthenticated(int code){
        if(code==REQ_BROADCAST){broadcastSigned();return;}
        try{
            ...
            if(code==REQ_PREVIEW){
                ...
                highFee.setChecked(false);sign.setEnabled(true);sign.setAlpha(1f);result.setText("");copy.setVisibility(View.GONE);broadcast.setVisibility(View.GONE);signedHex=null;signedTxid=null;
                ...
                signedHex=s.signedHex;signedTxid=s.txid;
                result.setText("SIGNED LOCALLY\nAddress: "+s.address+"\nTXID: "+s.txid+"\nFee: "+s.feeSats+" sats\n\nNothing has been broadcast. Final network approval is still required.");
                copy.setVisibility(View.VISIBLE);broadcast.setVisibility(View.VISIBLE);sign.setEnabled(false);sign.setAlpha(.35f);approvedRequest=null;
            }
    ...
    private void broadcastSigned(){
        final String raw=signedHex;final String expected=signedTxid;
        if(raw==null||raw.isEmpty())return;
        broadcast.setEnabled(false);broadcast.setAlpha(.4f);result.setText("Broadcasting signed transaction to Bitcoin mainnet…\nExpected TXID: "+expected);
        io.execute(()->{
            try{
                String txid=CounterpartyNetwork.broadcast(raw);
                runOnUiThread(()->{
                    broadcast.setEnabled(false);broadcast.setAlpha(.35f);copy.setVisibility(View.VISIBLE);
                    if(expected!=null&&!expected.equalsIgnoreCase(txid)) result.setText("BROADCAST RESPONSE MISMATCH\nLocal TXID: "+expected+"\nNetwork response: "+txid+"\nVerify before taking further action.");
                    else result.setText("BROADCAST ACCEPTED\nTXID: "+txid+"\n\nThe transaction has been submitted to the Bitcoin network. Counterparty will parse the embedded message after propagation/confirmation.");
                });
            }catch(Exception e){runOnUiThread(()->{broadcast.setEnabled(true);broadcast.setAlpha(1f);result.setText("Broadcast not accepted. Signed transaction remains local and can be retried.\n\n"+(e.getMessage()==null?e.getClass().getSimpleName():e.getMessage()));});}
        });
    }
