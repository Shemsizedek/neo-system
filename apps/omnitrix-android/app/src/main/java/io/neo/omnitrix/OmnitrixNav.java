...
/** Shared v2.6 consumer navigation. Never handles key material. */
public final class OmnitrixNav {
 private OmnitrixNav(){}
 public static LinearLayout bar(Activity a,String active){LinearLayout row=new LinearLayout(a);row.setOrientation(LinearLayout.HORIZONTAL);row.setGravity(Gravity.CENTER);row.setPadding(dp(a,3),dp(a,3),dp(a,3),dp(a,3));row.setBackgroundColor(Color.rgb(3,14,8));add(a,row,"Browser",active,()->launch(a,MainActivityV14.class));add(a,row,"Wallet",active,()->launch(a,WalletDashboardActivity.class));add(a,row,"Market",active,()->launch(a,MarketMatrixActivity.class));add(a,row,"Miner",active,()->launch(a,MinerControlActivity.class));add(a,row,"NEO",active,()->launch(a,ConsumerHomeActivity.class));return row;}
