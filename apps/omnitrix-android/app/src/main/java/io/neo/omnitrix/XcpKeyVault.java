 ...
 * Raw key material is never written to logs, GitHub, WebView storage, or network APIs.
 */
final class XcpKeyVault {
    ...
    boolean hasKey() {
        return prefs.contains(CT) && prefs.contains(IV);
    }

    String fingerprint() {
        return prefs.getString(FP, "");
    }
    ...
    /** Decrypts only long enough to prove the device/user can unlock the vault. */
    void verifyUnlocked() throws Exception {
    ...
    private static byte[] charsToAscii(char[] chars) {
        ...
        for (int i = 0; i < chars.length; i++) {
            char c = chars[i];
            if (c > 0x7f) throw new IllegalArgumentException("XCP Key must be Base58 text");
            out[i] = (byte)c;
        }
        return out;
    }
    ...
    private static void validateMainnetWif(byte[] wifAscii) throws Exception {
        ...
        try {
            ...
            byte[] check = doubleSha256(payload);
            try {
                for (int i = 0; i < 4; i++) if (decoded[payloadLen + i] != check[i]) throw new IllegalArgumentException("XCP Key checksum failed");
            } finally {
    ...
    private static String fingerprintFor(byte[] key) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < 4; i++) s.append(String.format("%02X", h[i]));
            return s.toString();
    ...
    private static byte[] decodeBase58(String input) {
        ...
        for (int i = 0; i < input.length(); i++) {
            int p = alphabet.indexOf(input.charAt(i));
            if (p < 0) throw new IllegalArgumentException("Invalid Base58 character");
            input58[i] = (byte)p;
        }
        ...
        Arrays.fill(input58, (byte)0); Arrays.fill(decoded, (byte)0);
        return out;
    }
}
