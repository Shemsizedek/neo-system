...
public class XcpKeyActivity extends Activity {
    ...
    private void authenticate(int requestCode) {
        ...
        if (km == null || !km.isDeviceSecure()) {
            new AlertDialog.Builder(this).setTitle("Device lock required").setMessage("Set a PIN, password, pattern, or supported biometric on Android before securing an XCP Key.").setPositiveButton("OK", null).show();
            return;
        }
        Intent intent = km.createConfirmDeviceCredentialIntent("Unlock Omnitrix", "Confirm your device identity for NEO-0001");
        if (intent != null) startActivityForResult(intent, requestCode); else onAuthenticated(requestCode);
    }

    @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        super.onActivityResult(requestCode, resultCode, data);
        if (resultCode == RESULT_OK) onAuthenticated(requestCode); else Toast.makeText(this,"Omnitrix remained locked",Toast.LENGTH_SHORT).show();
    }
    ...
    private void showImportDialog() {
        ...
        EditText key = new EditText(this); key.setHint("XCP Key"); key.setSingleLine(true); key.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD); key.setImportantForAutofill(View.IMPORTANT_FOR_AUTOFILL_NO_EXCLUDE_DESCENDANTS); box.addView(key, new LinearLayout.LayoutParams(-1,dp(56)));
        AlertDialog dialog = new AlertDialog.Builder(this).setTitle("Secure XCP Key").setView(box).setPositiveButton("Secure", null).setNegativeButton("Cancel", null).create();
        dialog.setOnShowListener(d -> dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
            char[] value = key.getText().toString().toCharArray(); key.setText("");
            try { vault.importXcpKey(value); dialog.dismiss(); Toast.makeText(this,"XCP Key secured",Toast.LENGTH_SHORT).show(); recreate(); }
            catch (android.security.keystore.UserNotAuthenticatedException e) { status.setText("Android requires authentication again before the vault can be written."); dialog.dismiss(); authenticate(REQ_IMPORT); }
            catch (Exception e) { new AlertDialog.Builder(this).setTitle("XCP Key not accepted").setMessage(e.getMessage()==null?"Invalid XCP Key":e.getMessage()).setPositiveButton("OK",null).show(); }
        }));
        dialog.show();
    }

    private void openBrowser() { Intent i = new Intent(this, MainActivityV14.class); i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP); startActivity(i); }
