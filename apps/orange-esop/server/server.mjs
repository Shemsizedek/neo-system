...
function authorize(req, allowed, roleTokens) {
  const requested = String(req.headers['x-neo-role'] || '');
  const supplied = String(req.headers['x-neo-role-token'] || '');
  ...
  return http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/health') {
        return json(res, 200, {service:'orange-esop', status:'ok'});
      }
      ...
      if (req.method === 'GET' && url.pathname === '/api/esop/public-summary') {
        ...
        return json(res, 200, {
          ...
          reconciliationStatus: data.reconciliation?.status || 'PENDING',
      ...
      if (req.method === 'POST' && url.pathname === '/api/esop/participants') {
        authorize(req, ['SECRETARY','PLAN_ADMINISTRATOR'], roleTokens);
      ...
      if (req.method === 'POST' && url.pathname === '/api/esop/certificate') {
        authorize(req, ['SECRETARY','PLAN_ADMINISTRATOR','ESOP_TRUSTEE'], roleTokens);
        return json(res, 409, {
          error:'AUTHORITATIVE_PLAN_DATA_NOT_AVAILABLE',
          message:'RCF-013 issuance remains disabled until allocation, vesting, valuation, and trustee approval are read from authoritative persisted records.'
        });
      }
      ...
      if (req.method === 'POST' && url.pathname === '/api/esop/statement') {
        ...
        return json(res, 409, {
          error:'AUTHORITATIVE_PLAN_DATA_NOT_AVAILABLE',
          message:'RCF-015 generation remains disabled until plan-year allocation, vesting, valuation, and stewardship data are read from authoritative persisted records.'
        });
      }
...
export function startOrangeEsopServer({port=Number(process.env.PORT||8794), store, roleTokens}={}) {
