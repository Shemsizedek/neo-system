...
export default function App(){
  ...
  const qrPayload=RECEIVE_ADDRESS && quote
    ? `neo-counter:${rail}:${quoted.toFixed(8)}:${paymentId}:${RECEIVE_ADDRESS}`
    : `neo-counter:configuration-required:${paymentId}`;
  ...
  const loadQuote=async(nextRail:Rail=rail)=>{
    ...
    if(nextRail==='USD'){
      const now=Date.now();
      setQuote({rail:'USD',asset:'USD',unitAmount:total/100,source:'Display currency',quotedAt:new Date(now).toISOString(),expiresAt:new Date(now+60_000).toISOString()});
  ...
  const observePayment=async()=>{
    ...
    try{
      const observation=await getReadOnlyRail(rail).observe({address:RECEIVE_ADDRESS,expectedAmount:quote.unitAmount,startedAt});
      if(!observation.detected){
        setStatus('awaiting_payment');
        setMessage(`No matching payment detected via ${observation.source}.`);
        return;
      }
      setStatus(observation.confirmed?'settled':'detected');
      setMessage(observation.confirmed?'Payment confirmed.':'Payment detected; awaiting confirmation.');
      if(observation.confirmed && !transactions.some(t=>t.id===paymentId)){
        setTransactions(t=>[{id:paymentId,total,rail,status:'settled',createdAt:new Date().toISOString(),reference:observation.reference},...t]);
      }
    }catch(error){
      setStatus('error');
      setMessage(error instanceof Error?error.message:'Network observation failed');
    }
  };
  ...
  return <div className="app-shell">
    ...
    {checkout&&<div className="modal-wrap"><div className="modal">
      ...
      <small className="disclaimer">Read-only network observation. No transaction composition, signing, broadcasting, custody, fiat transmission, or live card processing.</small>
    </div></div>}
  </div>
}
