...
export default function App(){
  ...
  const [rail,setRail]=useState<Rail>(gatewayIntent?.rail||'BTC');
  ...
  useEffect(()=>{
    if(gatewayIntent && checkout && total>0 && !gatewayBooted){
      ...
      void loadQuote(gatewayIntent.rail||'BTC');
    }
  ...
  const observePayment=async()=>{
    if(rail==='USD'){
      setStatus('settled');
      setTransactions(t=>[{id:paymentId,total,rail,status:'manual',createdAt:new Date().toISOString()},...t]);
